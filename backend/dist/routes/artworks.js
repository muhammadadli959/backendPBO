...
function transformArtworkRow(row) {
    return {
        id: String(row.id),
        title: row.title,
...
router.get("/", async (req, res) => {
    try {
        const result = await pool.query(`SELECT a.*, u.username as owner, c.name as category FROM artworks a
...
router.post("/", authMiddleware_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (!catRes.rows[0]) {
            const insertCat = await pool.query("INSERT INTO categories (name) VALUES ($1) ON CONFLICT (name) DO NOTHING RETURNING id", [category]);
            if (insertCat.rows[0]) {
                catRes = insertCat;
            }
            else {
        ...
        const result = await pool.query(`INSERT INTO artworks (title, description, image_url, artist_name, uploaded_by, uploaded_at, user_id, category_id)
...
router.put("/:id", authMiddleware_1.authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(`UPDATE artworks SET title = $1, description = $2, category_id = $3 WHERE id = $4 RETURNING *`, [
            title || artwork.title,
